(()=>{var he=Object.defineProperty;var i=(N,M)=>he(N,"name",{value:M,configurable:!0});(globalThis.webpackChunk=globalThis.webpackChunk||[]).push([["vendors-node_modules_github_paste-markdown_dist_index_esm_js-node_modules_github_quote-select-df2537"],{52769:(N,M,k)=>{"use strict";k.d(M,{Ld:()=>re});function b(f,y){var E,C,O;const q=f.value.slice(0,(E=f.selectionStart)!==null&&E!==void 0?E:void 0),Y=f.value.slice((C=f.selectionEnd)!==null&&C!==void 0?C:void 0);let J=!0;f.contentEditable="true";try{J=document.execCommand("insertText",!1,y)}catch{J=!1}if(f.contentEditable="false",J&&!f.value.slice(0,(O=f.selectionStart)!==null&&O!==void 0?O:void 0).endsWith(y)&&(J=!1),!J){try{document.execCommand("ms-beginUndoUnit")}catch{}f.value=q+y+Y;try{document.execCommand("ms-endUndoUnit")}catch{}f.dispatchEvent(new CustomEvent("change",{bubbles:!0,cancelable:!0}))}}i(b,"insertText");const _=new WeakMap;function h(f){const{currentTarget:y}=f,E=f.code==="KeyV"&&(f.ctrlKey||f.metaKey)&&f.shiftKey;(E||E&&f.altKey)&&_.set(y,!0)}i(h,"setSkipFormattingFlag");function v(f){const{currentTarget:y}=f;_.delete(y)}i(v,"unsetSkipFormattedFlag");function e(f){var y;return(y=_.get(f))!==null&&y!==void 0?y:!1}i(e,"shouldSkipFormatting");function o(f,...y){f.addEventListener("keydown",h);for(const E of y)E(f);f.addEventListener("paste",v)}i(o,"installAround");function t(f){f.removeEventListener("keydown",h),f.removeEventListener("paste",v)}i(t,"uninstall$5");function n(f){f.addEventListener("paste",l)}i(n,"install$4");function r(f){f.removeEventListener("paste",l)}i(r,"uninstall$4");function l(f){const y=f.clipboardData,{currentTarget:E}=f;if(e(E)||!y||!c(y))return;const C=f.currentTarget;if(!(C instanceof HTMLTextAreaElement))return;let O=y.getData("text/plain");const q=y.getData("text/html"),Y=q.replace(/\u00A0/g," ").replace(/\uC2A0/g," ");if(!q||(O=O.trim(),!O))return;const Z=new DOMParser().parseFromString(Y,"text/html"),Q=Z.createTreeWalker(Z.body,NodeFilter.SHOW_ELEMENT,fe=>fe.parentNode&&s(fe.parentNode)?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_ACCEPT),oe=u(O,Q);oe!==O&&(f.stopPropagation(),f.preventDefault(),b(C,oe))}i(l,"onPaste$4");function u(f,y){var E;let C=y.firstChild(),O=f,q=0,Y=0;const J=1e4;for(;C&&Y<J;){Y++;const Z=s(C)?(C.textContent||"").replace(/[\t\n\r ]+/g," "):((E=C.firstChild)===null||E===void 0?void 0:E.wholeText)||"";if(a(Z)){C=y.nextNode();continue}const Q=O.indexOf(Z,q);if(Q>=0)if(s(C)){const oe=m(C,Z);O=O.slice(0,Q)+oe+O.slice(Q+Z.length),q=Q+oe.length}else q=Q+Z.length;C=y.nextNode()}return Y===J?f:O}i(u,"convertToMarkdown");function a(f){return!f||(f==null?void 0:f.trim().length)===0}i(a,"isEmptyString");function s(f){var y;return((y=f.tagName)===null||y===void 0?void 0:y.toLowerCase())==="a"&&f.hasAttribute("href")}i(s,"isLink");function c(f){return f.types.includes("text/html")}i(c,"hasHTML");function m(f,y){const E=f.href||"";let C="";return g(f)?C=y:d(f)||p(E,y)?C=E:C=`[${y}](${E})`,C}i(m,"linkify$2");function d(f){return f.className.indexOf("commit-link")>=0||!!f.getAttribute("data-hovercard-type")&&f.getAttribute("data-hovercard-type")!=="user"}i(d,"isSpecialLink");function p(f,y){return f=f.slice(-1)==="/"?f.slice(0,-1):f,y=y.slice(-1)==="/"?y.slice(0,-1):y,f.toLowerCase()===y.toLowerCase()}i(p,"areEqualLinks");function g(f){var y;return((y=f.textContent)===null||y===void 0?void 0:y.slice(0,1))==="@"&&f.getAttribute("data-hovercard-type")==="user"}i(g,"isUserMention");function w(f){f.addEventListener("dragover",$),f.addEventListener("drop",I),f.addEventListener("paste",z)}i(w,"install$3");function x(f){f.removeEventListener("dragover",$),f.removeEventListener("drop",I),f.removeEventListener("paste",z)}i(x,"uninstall$3");function I(f){const y=f.dataTransfer;if(!y||se(y)||!ee(y))return;const E=te(y);if(!E.some(T))return;f.stopPropagation(),f.preventDefault();const C=f.currentTarget;C instanceof HTMLTextAreaElement&&b(C,E.map(V).join(""))}i(I,"onDrop$1");function $(f){const y=f.dataTransfer;y&&(y.dropEffect="link")}i($,"onDragover$1");function z(f){const{currentTarget:y}=f;if(e(y))return;const E=f.clipboardData;if(!E||!ee(E))return;const C=te(E);if(!C.some(T))return;f.stopPropagation(),f.preventDefault();const O=f.currentTarget;O instanceof HTMLTextAreaElement&&b(O,C.map(V).join(""))}i(z,"onPaste$3");function V(f){return T(f)?`
![](${f})
`:f}i(V,"linkify$1");function se(f){return Array.from(f.types).indexOf("Files")>=0}i(se,"hasFile$1");function ee(f){return Array.from(f.types).indexOf("text/uri-list")>=0}i(ee,"hasLink");function te(f){return(f.getData("text/uri-list")||"").split(`\r
`)}i(te,"extractLinks");const P=/\.(gif|png|jpe?g)$/i;function T(f){return P.test(f)}i(T,"isImageLink");function A(f){f.addEventListener("paste",S)}i(A,"install$2");function R(f){f.removeEventListener("paste",S)}i(R,"uninstall$2");function S(f){const{currentTarget:y}=f;if(e(y))return;const E=f.clipboardData;if(!E||!F(E))return;const C=f.currentTarget;if(!(C instanceof HTMLTextAreaElement))return;const O=E.getData("text/plain");if(!O||!H(O)||D(C))return;const q=C.value.substring(C.selectionStart,C.selectionEnd);!q.length||H(q.trim())||(f.stopPropagation(),f.preventDefault(),b(C,U(q,O)))}i(S,"onPaste$2");function F(f){return Array.from(f.types).includes("text/plain")}i(F,"hasPlainText");function D(f){const y=f.selectionStart||0;return y>1?f.value.substring(y-2,y)==="](":!1}i(D,"isWithinLink");function U(f,y){return`[${f}](${y})`}i(U,"linkify");function H(f){return/^https?:\/\//i.test(f)}i(H,"isURL");function B(f){f.addEventListener("dragover",ce),f.addEventListener("drop",G),f.addEventListener("paste",ie)}i(B,"install$1");function ne(f){f.removeEventListener("dragover",ce),f.removeEventListener("drop",G),f.removeEventListener("paste",ie)}i(ne,"uninstall$1");function G(f){const y=f.dataTransfer;if(!y||le(y))return;const E=L(y);if(!E)return;f.stopPropagation(),f.preventDefault();const C=f.currentTarget;C instanceof HTMLTextAreaElement&&b(C,E)}i(G,"onDrop");function ce(f){const y=f.dataTransfer;y&&(y.dropEffect="copy")}i(ce,"onDragover");function ie(f){const{currentTarget:y}=f;if(e(y)||!f.clipboardData)return;const E=L(f.clipboardData);if(!E)return;f.stopPropagation(),f.preventDefault();const C=f.currentTarget;C instanceof HTMLTextAreaElement&&b(C,E)}i(ie,"onPaste$1");function le(f){return Array.from(f.types).indexOf("Files")>=0}i(le,"hasFile");function ae(f){const y="\xA0";return(f.textContent||"").trim().replace(/\|/g,"\\|").replace(/\n/g," ")||y}i(ae,"columnText");function de(f){return Array.from(f.querySelectorAll("td, th")).map(ae)}i(de,"tableHeaders");function ue(f){const y=Array.from(f.querySelectorAll("tr")),E=y.shift();if(!E)return"";const C=de(E),O=C.map(()=>"--"),q=`${C.join(" | ")}
${O.join(" | ")}
`,Y=y.map(J=>Array.from(J.querySelectorAll("td")).map(ae).join(" | ")).join(`
`);return`
${q}${Y}

`}i(ue,"tableMarkdown");function L(f){if(Array.from(f.types).indexOf("text/html")===-1)return;const y=f.getData("text/html");if(!/<table/i.test(y))return;let O=new DOMParser().parseFromString(y,"text/html").querySelector("table");if(O=!O||O.closest("[data-paste-markdown-skip]")?null:O,!O)return;const q=ue(O);return y.replace(/<meta.*?>/,"").replace(/<table[.\S\s]*<\/table>/,`
${q}`)}i(L,"generateText");function j(f){f.addEventListener("paste",K)}i(j,"install");function W(f){f.removeEventListener("paste",K)}i(W,"uninstall");function K(f){const{currentTarget:y}=f;if(e(y))return;const E=f.clipboardData;if(!E||!X(E))return;const C=f.currentTarget;if(!(C instanceof HTMLTextAreaElement))return;const O=E.getData("text/x-gfm");!O||(f.stopPropagation(),f.preventDefault(),b(C,O))}i(K,"onPaste");function X(f){return Array.from(f.types).indexOf("text/x-gfm")>=0}i(X,"hasMarkdown");function re(f){return o(f,B,w,A,j,n),{unsubscribe:()=>{t(f),ne(f),r(f),x(f),R(f),W(f)}}}i(re,"subscribe")},55498:(N,M,k)=>{"use strict";k.d(M,{I:()=>a,p:()=>u});function b(s){const c=s.parentNode;if(c===null||!(c instanceof HTMLElement))throw new Error;let m=0;c instanceof HTMLOListElement&&c.start!==1&&(m=c.start-1);const d=c.children;for(let p=0;p<d.length;++p)if(d[p]===s)return m+p;return m}i(b,"indexInList");function _(s){if(s instanceof HTMLAnchorElement&&s.childNodes.length===1){const c=s.childNodes[0];if(c instanceof HTMLImageElement)return c.src===s.href}return!1}i(_,"skipNode");function h(s){return s.nodeName==="IMG"||s.firstChild!=null}i(h,"hasContent");function v(s){return s.nodeName==="INPUT"&&s instanceof HTMLInputElement&&s.type==="checkbox"}i(v,"isCheckbox");let e=0;function o(s){const c=s.childNodes[0],m=s.childNodes[1];return c&&s.childNodes.length<3?(c.nodeName==="OL"||c.nodeName==="UL")&&(!m||m.nodeType===Node.TEXT_NODE&&!(m.textContent||"").trim()):!1}i(o,"nestedListExclusive");function t(s){return s.replace(/&/g,"&amp;").replace(/'/g,"&apos;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}i(t,"escapeAttribute");const n={INPUT(s){return s instanceof HTMLInputElement&&s.checked?"[x] ":"[ ] "},CODE(s){const c=s.textContent||"";return s.parentNode&&s.parentNode.nodeName==="PRE"?(s.textContent=`\`\`\`
${c.replace(/\n+$/,"")}
\`\`\`

`,s):c.indexOf("`")>=0?`\`\` ${c} \`\``:`\`${c}\``},P(s){const c=document.createElement("p"),m=s.textContent||"";return c.textContent=m.replace(/<(\/?)(pre|strong|weak|em)>/g,"\\<$1$2\\>"),c},STRONG(s){return`**${s.textContent||""}**`},EM(s){return`_${s.textContent||""}_`},DEL(s){return`~${s.textContent||""}~`},BLOCKQUOTE(s){const c=(s.textContent||"").trim().replace(/^/gm,"> "),m=document.createElement("pre");return m.textContent=`${c}

`,m},A(s){const c=s.textContent||"",m=s.getAttribute("href");return/^https?:/.test(c)&&c===m?c:m?`[${c}](${m})`:c},IMG(s){const c=s.getAttribute("alt")||"",m=s.getAttribute("src");if(!m)throw new Error;const d=s.hasAttribute("width")?` width="${t(s.getAttribute("width")||"")}"`:"",p=s.hasAttribute("height")?` height="${t(s.getAttribute("height")||"")}"`:"";return d||p?`<img alt="${t(c)}"${d}${p} src="${t(m)}">`:`![${c}](${m})`},LI(s){const c=s.parentNode;if(!c)throw new Error;let m="";o(s)||(c.nodeName==="OL"?e>0&&!c.previousSibling?m=`${b(s)+e+1}\\. `:m=`${b(s)+1}. `:m="* ");const d=m.replace(/\S/g," "),p=(s.textContent||"").trim().replace(/^/gm,d),g=document.createElement("pre");return g.textContent=p.replace(d,m),g},OL(s){const c=document.createElement("li");return c.appendChild(document.createElement("br")),s.append(c),s},H1(s){const c=parseInt(s.nodeName.slice(1));return s.prepend(`${Array(c+1).join("#")} `),s},UL(s){return s}};n.UL=n.OL;for(let s=2;s<=6;++s)n[`H${s}`]=n.H1;function r(s){const c=document.createNodeIterator(s,NodeFilter.SHOW_ELEMENT,{acceptNode(p){return p.nodeName in n&&!_(p)&&(h(p)||v(p))?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_SKIP}}),m=[];let d=c.nextNode();for(;d;)d instanceof HTMLElement&&m.push(d),d=c.nextNode();m.reverse();for(const p of m)p.replaceWith(n[p.nodeName](p))}i(r,"insertMarkdownSyntax");function l(s,c){const m=s.startContainer;if(!m||!m.parentNode||!(m.parentNode instanceof HTMLElement))throw new Error("the range must start within an HTMLElement");const d=m.parentNode;let p=s.cloneContents();if(c){const x=p.querySelector(c);x&&(p=document.createDocumentFragment(),p.appendChild(x))}e=0;const g=d.closest("li");if(d.closest("pre")){const x=document.createElement("pre");x.appendChild(p),p=document.createDocumentFragment(),p.appendChild(x)}else if(g&&g.parentNode&&(g.parentNode.nodeName==="OL"&&(e=b(g)),!p.querySelector("li"))){const x=document.createElement("li");if(!g.parentNode)throw new Error;const I=document.createElement(g.parentNode.nodeName);x.appendChild(p),I.appendChild(x),p=document.createDocumentFragment(),p.appendChild(I)}return p}i(l,"extractFragment");class u{constructor(){this.selection=window.getSelection()}closest(c){const m=this.range.startContainer,d=m instanceof Element?m:m.parentElement;return d?d.closest(c):null}get active(){var c;return(((c=this.selection)===null||c===void 0?void 0:c.rangeCount)||0)>0}get range(){var c;return((c=this.selection)===null||c===void 0?void 0:c.rangeCount)?this.selection.getRangeAt(0):new Range}set range(c){var m,d;(m=this.selection)===null||m===void 0||m.removeAllRanges(),(d=this.selection)===null||d===void 0||d.addRange(c)}get selectionText(){var c;return((c=this.selection)===null||c===void 0?void 0:c.toString().trim())||""}get quotedText(){return`> ${this.selectionText.replace(/\n/g,`
> `)}
